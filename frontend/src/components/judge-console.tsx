...
export function JudgeConsole() {
  ...
  return <div className="grid-2">
    <section className="panel featured">
      ...
      <div className="form-actions">{(mode === "push" || recipientPayment) && <button className="button button-outline" onClick={approve} disabled={!walletReady || !hasGas || !!busy || !value || !enoughBalance || enoughAllowance}>{busy === "approve" ? "Simulating + approving…" : enoughAllowance ? "Approved" : `Approve ${amount || "0"} gTST`}</button>}<button className="button button-light" onClick={mode === "pull" && !recipientPayment ? createPullRequestLink : createEscrow} disabled={!!busy || !value || (mode === "pull" && !recipientPayment ? !isAddress(recipient) : (!walletReady || !hasGas || !enoughBalance))}>{busy === "escrow" ? recipientPayment ? "Approving + paying…" : "Preflighting + creating…" : mode === "push" ? "Fund Push escrow" : programId !== ZERO_HASH ? "Deposit / Pay campaign" : isPaymentRequest ? "Deposit / Pay" : "Create Pull link"}</button></div>
      {!enoughBalance && walletReady && <p className="notice" style={{marginTop:18}}>Mint enough demo gTST first.</p>}
      {enoughBalance && !enoughAllowance && walletReady && !recipientPayment && mode === "push" && <p className="notice" style={{marginTop:18}}>Approve the exact gTST amount before funding Push escrow.</p>}
